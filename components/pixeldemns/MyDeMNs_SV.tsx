...
function MyDeMNs_SV({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `https://api.tzkt.io/v1/tokens/balances?active=true&token.contract=KT1U6EHmNxJTkvaWJ4ThczG4FSDaHC21ssvi&account=${myuserAddress}&balance=1&select=token.metadata.name`;
    ...
    <>
      {showData && demnData.map((post, index) => {
        ...
        if (pic.includes('PixelDeMN')) {
          ...
          const ipfsimage = `/images/Series V/${fixedPic}.png`;
          return (
            <span key={index}>
              <p>Series V - PixelDeMN {fixedPic}</p>
