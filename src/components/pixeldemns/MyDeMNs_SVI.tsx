...
function MyDeMNs_SVI({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `https://api.tzkt.io/v1/tokens/balances?active=true&token.contract=KT1DxnxEvjxJ5yw9SWS8B7HZVAS9md3VqMeY&account=${myuserAddress}&balance=1&select=token.metadata.name`;
    ...
    <>
      {showData && demnData.map((post, index) => {
        ...
        if (pic.includes('PixelDeMN')) {
          ...
          const ipfsimage = `/images/Series VI/${fixedPic}.png`;
          return (
            <span key={index}>
              <p>Series VI - PixelDeMN {fixedPic}</p>
