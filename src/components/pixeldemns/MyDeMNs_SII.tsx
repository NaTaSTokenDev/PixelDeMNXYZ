...
function MyDeMNs_SII({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `https://api.tzkt.io/v1/tokens/balances?active=true&token.contract=KT1QctVjmHzMTBBmHLwoYToaodEx7n1BXG1b&account=${myuserAddress}&balance=1&select=token.metadata.name`;        
    ...
    <>
      ...
      {showData && demnData.map((post, index) => {
        ...
        if (pic.includes('PixelDeMN')) {
          ...
          const ipfsimage = `/images/Series II/${fixedPic}.png`;
          console.log(ipfsimage, "sii")
          return (
            <span key={index}>
              <p>Series II - PixelDeMN {fixedPic}</p>
